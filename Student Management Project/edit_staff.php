...
<div class="container bg-info" id="formsetting">
    <h3 class="text-center text-white pb-4 pt-3 font-weight-bold">Edit Staff Details</h3>
    
<?php
if(isset($_GET['edit_staff']))
{
$edit_staff_id=$_GET['edit_staff'];
$query="select * from staff_detail where staff_id='$edit_staff_id'";
...
while($row=mysqli_fetch_assoc($run))
{
    ...
    <div class="row">
        <div class="col-md-5 col-sm-5 col-12 m-auto">
            <div class="form-group">
             ...
             <div class="form-group">
                ...
                <input type="text" name="qual" placeholder="Enter Your Qualification" class="form-control" value="<?php echo $row['qual']; ?>">
             </div>
             <div class="form-group">
             ...
             <div class="form-group">
                ...
                <input type="email" name="email1" placeholder="Enter Your Email" class="form-control" value="<?php echo $row['email1']; ?>">
             </div>
             <div class="form-group">
        ...
        <div class="form-group">
             ...
             <div class="form-group">
                ...
                <input type="text" name="exp" placeholder="Enter Your Experience" class="form-control" value="<?php echo $row['exp']; ?>">
             </div>
             <div class="form-group">
   ...
   <input type="submit" name="update1" value="Update detail" class="btn btn-success px-5 mt-2">
</div>
</form>  
</div>
</div>
...
$image_query="select * from staff_detail where staff_id='$edit_staff_id'";
$run=mysqli_query($conn,$image_query);
while($row=mysqli_fetch_assoc($run)){
    $img=$row['photo'];
}

unlink('staff_image/'.$img);
...
if(!$image_type == 'image/jpg' or !$image_type == 'image/png'){
   ...
   $sql="update staff_detail set fname='$fname',lname='$lname',father_name='$fathername',email1='$email',mobile='$mobile',birthdate='$birthdate',gender='$gender',district='$district',city='$city',state='$state',nationality='$nationality',photo='$image' where staff_id='$edit_staff_id'";
$run=mysqli_query($conn,$sql);
if($run){
 echo "<script>window.open('view_staff.php?update_success=Student data updated successfully')</script>";
}
else{

    echo "<script>window.open('view_staff.php?update_error=unable to update data.Please try again','_self')</script>";
